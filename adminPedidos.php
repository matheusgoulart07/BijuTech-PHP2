...
if ($_POST && isset($_POST['id_compra'])) {
    ...
    if (isset($_POST['novo_status']) && $_POST['novo_status'] != '') {
        $novo_status = strtolower($_POST['novo_status']);
    ...
    if (!empty($updates)) {
        ...
        $stmtUser = $pdo->prepare("SELECT u.nome, u.email FROM usuario u JOIN compra c ON c.fk_usuario = u.id_usuario WHERE c.id_compra = :id_compra");
...
$stmtPedidos = $pdo->prepare("SELECT c.id_compra, c.status, c.data, c.acrescimo_total, u.nome AS cliente_nome, u.email AS cliente_email FROM compra c JOIN usuario u ON c.fk_usuario = u.id_usuario $whereFiltro ORDER BY c.data DESC");
    ...
    <main class="flex-fill">
        <div class="admin-container">
            <div class="admin-content">
                <h1 class="admin-title">Painel Administrativo - Pedidos</h1>

                <?php if (isset($msg_sucesso)): ?>
                    <p class="success-message"><?= htmlspecialchars($msg_sucesso) ?></p>
                <?php endif; ?>
                ...
                <div class="table-responsive-wrapper">
                    ...
                    <?php else: ?>
                        <table class="admin-table">
                            <thead>
                                <tr><th>ID</th><th>Cliente</th><th>Email</th><th>Data</th><th>Status</th><th>Acrésc./Desc.</th><th>Total</th><th>Itens</th><th>Ações</th></tr>
                            </thead>
                            <tbody>
                            <?php foreach ($pedidos as $pedido): ?>
                                <?php
                                    $stmtItens = $pdo->prepare("SELECT p.nome, cp.quantidade, cp.valor_unitario FROM compra_produto cp JOIN produto p ON cp.fk_produto = p.id_produto WHERE cp.fk_compra = :id_compra");
                                ...
                                <tr>
                                    ...
                                    <td>R$ <?= number_format($pedido['acrescimo_total'], 2, ',', '.'); ?></td>
